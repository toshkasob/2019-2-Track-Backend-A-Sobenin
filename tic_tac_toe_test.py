...
class TestTicTacToe(unittest.TestCase):
    ...
    def test_change_player(self):
        game = TicTacToe()
        player_id_start = game.player_id
        for step in range(9):
            self.assertEqual(game.player_id, player_id_start * ((-1) ** step))
            game.change_player()
    ...
    def test_change_board(self):
        for player_id_cur in [-1, 1]:
            marker = "x" if player_id_cur == 1 else "o"
            for position in range(9):
                game = TicTacToe()
                game.player_id = player_id_cur
